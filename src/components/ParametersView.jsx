...
export default function ParametersView() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await db.getTelemetry();
      setTelemetry(data);
    ...
    return (
      <div className="glass-panel p-12 rounded-2xl border text-center space-y-4">
        ...
        <div className="max-w-md mx-auto space-y-2">
          <h3 className="text-xl font-bold font-mono">No Parameter Telemetry Available</h3>
          <p className="text-xs text-slate-400 font-sans leading-relaxed">
            Telemetry sensor streams (Engine EGT, vibration IPS, hydraulic PSI, rotor RPM) will populate automatically when the dataset is imported.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="glass-panel p-5 rounded-2xl border">
        <h2 className="text-lg font-bold font-mono">Sensor Telemetry Explorer</h2>
      </div>
    </div>
  );
}
